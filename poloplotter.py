...
def ohlc_to_dict(chart_data):
    ...
    for item in chart_data:
        ...
        weightedAverage_list.append(float(item['weightedAverage']))
        low_list.append(float(item['low']))
        quoteVolume_list.append(float(item['quoteVolume']))
        volume_list.append(float(item['volume']))
        date_list.append(to_dt(item['date']))
        high_list.append(float(item['high']))
        
    OHLC_dict = {
        'close': np.asarray(close_list),
        'open': np.asarray(open_list),
        'weightedAverage': np.asarray(weightedAverage_list),
        'low': np.asarray(low_list),
        'quoteVolume': np.asarray(quoteVolume_list),
